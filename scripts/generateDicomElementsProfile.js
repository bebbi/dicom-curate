...
async function main() {
    try {
        ...
        const elementsToAnonymize = await fetchDicomStandard('confidentiality_profile_attributes.json');
        const anonymizeSet = new Set(elementsToAnonymize.map(element => element.tag));
...
`;
        ...
        await writeFile(
            join(configDir, 'elementNamesToAlwaysKeep.js'),
            jsContent
        );
