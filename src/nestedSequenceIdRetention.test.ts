...
describe('Nested Sequence ID Retention Tests', () => {
  ...
  it('preserves Other Patient IDs Sequence structure when retainPatientCharacteristicsOption includes eligible fields', () => {
    const testData = createTestDicomData()

    // Add the Other Patient IDs Sequence with fields that are eligible for retention
    testData.dict['00101002'] = {
      // OtherPatientIDsSequence
      vr: 'SQ',
      Value: [
        {
          '00100020': { vr: 'LO', Value: ['PATIENT_ID_1'] }, // PatientID (not eligible for retention)
          '00100021': { vr: 'LO', Value: ['ISSUER_1'] }, // IssuerOfPatientID (not eligible for retention)
          '00101010': { vr: 'AS', Value: ['025Y'] }, // PatientAge (eligible for retention)
          '00101030': { vr: 'DS', Value: ['70.5'] }, // PatientWeight (eligible for retention)
        },
      ],
    }
    ...
    // When patient characteristics are explicitly retained, the sequence should be preserved
    expect(naturalized.OtherPatientIDsSequence).toBeDefined()
    expect(naturalized.OtherPatientIDsSequence).toHaveLength(1)

    const firstItem = naturalized.OtherPatientIDsSequence[0]
    expect(firstItem).toBeDefined()

    // Eligible fields should be retained when explicitly configured
    expect(firstItem.PatientAge).toEqual('025Y')
    expect(firstItem.PatientWeight).toEqual('70.5')

    // Non-eligible fields should still be processed according to their basicProfile
    expect(firstItem.PatientID).toEqual('REDACTED') // Z/D rule -> dummy value
    expect(firstItem.IssuerOfPatientID).toBeUndefined() // X rule -> deleted
  })
