...
export type TCurateOneArgs = {
  fileInfo: TFileInfo
  fileIndex?: number
  outputTarget: { url?: string; directory?: FileSystemDirectoryHandle | undefined; token?: string }
...
> {
  ...
  if (!mappingOptions.skipWrite) {
    ...
    // If directory target provided, write to it (filesystem API)
    if (outputTarget?.directory) {
      const subDirectoryHandle = await createNestedDirectories(outputTarget.directory, dirPath)
